...
def build_tables(raw_data, replaces, template_env, config):
    for table in raw_data:
        table_replaces = replaces.copy()
        table_replaces["datatable"] = raw_data[table]
...
def build_html(template_env, replaces, output_file, config):
    ...
    tables = [{"name": table, "url": f"{table}.html"} for table in config["tables"]]
...
def render_html(raw_data, replaces, config):
    ...
    # Build tables from raw data
    build_tables(raw_data, replaces, template_env, config)
